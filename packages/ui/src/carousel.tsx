...
import { RxCaretLeft, RxCaretRight, RxPause, RxPlay } from "react-icons/rx";
...
>(({ className, variant = "fullGhost", size = "icon", ...props }, ref) => {
...
const CarouselAutoplayToggle = React.forwardRef<
  ...
  React.ComponentProps<typeof Button>
>(({ className, variant = "fullGhost", size = "icon", ...props }, ref) => {
  const { orientation, isPlaying, setIsPlaying } = useCarousel();
  ...
  return (
    ...
    >
      {isPlaying ? (
        <RxPause className="h-4 w-4" />
      ) : (
        <RxPlay className="h-4 w-4" />
      )}
